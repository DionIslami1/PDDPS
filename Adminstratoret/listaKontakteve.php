						...
						<section id="four" class="wrapper alt style1">
							...
							<div class="table-wraper" style="border-top: 10%" >
                                <form action="" method="post">
									<table>
										<tr>
											<h2>Kërko kontaktet për fshirje</h2>
										</tr>
										<tr>
											<td> 
												Shkruaj:
											...
											<td>
												<input type="text" name="term" placeholder="Subijekti ose Email-in"/>
											</td>
									...
									<table style="width: 100%; white-space: nowrap;">
										...
										<?php
											if (!empty($_REQUEST['term'])) {
												...
												($conn,$_REQUEST['term']);     
												$sql= mysqli_query ($conn32, "CALL zgjedhTermKontakt('$term')"); 
											while($row = mysqli_fetch_array($sql)) { 		
													echo "<tr>";
													echo "<td>".$row['subijekti']."</td>";	
													echo "<td>".$row['mesazhi']."</td>";
													echo "<td>".$row['emaili']."</td>";
													echo "<td><a href=\"fshiKontakt.php?id_kontakti=$row[id_kontakti]
													\" onClick=\"return confirm('A jeni të sigurt se dëshironi të fshini Kontaktin?')\" 
