...
<html>
	<head>
		<title>Lista e Universiteteve</title>
		<meta charset="utf-8" />
						...
						<section id="four" class="wrapper alt style1">
							...
							<div class="table-wraper" style="border-top: 10%" >
                                <form action="" method="post">
									<table>
										<tr>
											<h2>Kërko të dhënat e Universitetit për modifikim ose fshirje</h2>
										</tr>
										<tr>
											<td> 
												Shkruaj:
											...
											<td>
												<input type="text" name="term" placeholder="Universitetin"/>
											</td>
									...
									<table style="width: 100%; white-space: nowrap;">
										...
										<?php
											if (!empty($_REQUEST['term'])) {
												...
												($conn,$_REQUEST['term']);     
												$sql= mysqli_query ($conn31, "CALL zgjedhTermUniversitetet('$term')"); 
											while($row = mysqli_fetch_array($sql)) { 		
													echo "<tr>";
													echo "<td>".$row['universiteti']."</td>";		
													echo "<td><a href=\"updateUniversitet.php?id_uni=$row[id_uni]\" class='button primary'>
													Modifiko</a></td>";
													echo "<td><a href=\"fshiUniversitet.php?id_uni=$row[id_uni]
													\" onClick=\"return confirm('A jeni të sigurt së dëshironi të fshini Universitetin?')\" 
